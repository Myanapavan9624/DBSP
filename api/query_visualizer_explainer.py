    ...
    try:
        ...
        if "Plan" in plan:
            ...
            while queue:
                ...
                if "Plans" in curr:
                    depth = curr["depth"] + 1
                    children = [child for child in curr["Plans"] if child not in visited]
...
def craft_explanation_string(explanation, node_type, child_names, curr_name):
    ...
    #     )
    try:
        explanation += f"{node_type} "
        
        if node_type in {"Hash", "Sort", "Incremental Sort", "Gather Merge", "Merge", "Aggregate"}:
            explanation += f"{child_names[0]['id']} as {curr_name}."
        elif node_type in {"Hash Join", "Nested Loop", "Merge Join"}:
            if node_type == "Nested Loop":
                explanation += f"Join between {child_names[0]['Node Type']} {child_names[0]['id']} (outer) and {child_names[1]['Node Type']} {child_names[1]['id']} (inner) as {curr_name}."
        else:
            try:
                explanation += f"{child_names[0]['id']} as {curr_name}."
            except KeyError:
                explanation += f"on {child_names['Relation Name']} as {curr_name}."
